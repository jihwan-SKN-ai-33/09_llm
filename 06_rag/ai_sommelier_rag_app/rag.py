...
def _load_project_environment() -> None:
    ...
    # usecwd=Ture -> 현재 폴더를 탐색 시작 위치로 지정
...
PINECONE_INDEX_NAME = os.getenv("SOMMELIER_PINECONE_INDEX", "winemag-review-data").strip() or "winemag_review_data"
...
def describe_dish_flavor(payload: dict[str, Any]) -> Runnable:
    """이미지 URL을 만들어서 영어 한 문장의 풍미 query르 생성한 Runnable 반환"""
    ...
    # prompt(이미지) -> LLM -> parser chain(문자열) 구성
...
def search_wines(dish_flavor: str) -> dict[str, str]:
    ...
    # 최종 답변 생성 LLM(== Generation)에 전달할 query
    return {
